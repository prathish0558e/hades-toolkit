...
class APIProvider:
    ...
    def __init__(self, cc, target, mode, delay=0):
        # Construct the absolute path to api.json, which is in the parent directory
        api_file_path = os.path.join(os.path.dirname(__file__), '..', 'api.json')
        try:
            with open(api_file_path, 'r') as f:
                PROVIDERS = json.load(f)
        except Exception:
            PROVIDERS = requests.get(
                "https://raw.githubusercontent.com/prathish0558e/hades-toolkit/main/tools/sms_bomber/api.json"
            ).json()
        self.config = None
        self.cc = cc
        self.target = target
